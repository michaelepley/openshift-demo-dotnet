...
[[ -v CONFIGURATION_RESOURCES_GITHUB_COMPLETED ]] && echo "	--> Using resources available configuration" && { return || exit ; }
...
: ${GITHUB_AUTHORIZATION_TOKEN_OPENSHIFT_DEMO_PLAINTEXT:=`echo ${GITHUB_AUTHORIZATION_TOKEN_OPENSHIFT_DEMO_CIPHERTEXT} | openssl enc -d -a -aes-256-cbc -k "${SCRIPT_ENCRYPTION_KEY}"`} || { echo "FAILED: Could not validate the github token" && exit 1; }
